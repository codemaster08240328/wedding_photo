...
class CalendarView extends Component {
  ...
  selectedDay = async day => {
    try {
      const { markedData } = this.state;
      const { dateString } = day;

      if (!!markedData[dateString] && markedData[dateString].reason === 0) {
        this.refs.toast.show("Booked Date couldn't be deleted.", 2000);
        return;
      }
      ...
      if (selectedDay in this.state.markedData) {
        let deletingObj = this.props.unavailable_date.filter(
          item => item.unavl_date === selectedDay
        );
        ...
        if (result1 && !result1.error) {
          ...
          this.refs.toast.show("Selected Date couldn't be deleted.", 2000);
        }
      } else {
        const param = {
          photog_id: this.props.user.photog_id,
